...
const FLEET_FEATURES = [
  "Airport Pickup", "Airport Dropoff", "24/7 Available", "Corporate Hire",
  "Wedding Car", "Long Distance", "Self Drive", "Chauffeur Service",
  "Intercity", "School Run", "Event Hire", "VIP Service",
  "Night Travel", "Group Travel", "Luggage Space", "Pet Friendly",
];
...
export default function MyFleetPage() {
  ...
  const { data: vendorData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: venue } = await (supabase.from("venues") as any)
        .select("id,name,images,filters,phone,whatsapp,instagram,contact_email,website,videos,address")
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["my-fleet"] });
      setSaved(true);
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#F7F5FA", maxWidth: 480, margin: "0 auto", paddingBottom: 40 }}>
      ...
      <div style={{ backgroundColor: "#FFFFFF", padding: "16px", display: "flex", alignItems: "center", gap: 10, borderBottom: "1px solid #F2EEF9", position: "sticky", top: 0, zIndex: 40 }}>
        ...
        <div style={{ flex: 1 }}>
          <h1 style={{ fontSize: 18, fontWeight: 900, color: "#0A0A0A", margin: 0, fontFamily: "var(--font-display,Syne,sans-serif)" }}>My Fleet</h1>
          <p style={{ fontSize: 11, color: "#9E9E9E", margin: 0 }}>Edit your car rental profile</p>
        </div>
        <button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}
          style={{ padding: "8px 18px", borderRadius: 12, border: "none", backgroundColor: saveMutation.isPending ? "#9E9E9E" : ACCENT, color: "#FFFFFF", fontSize: 13, fontWeight: 700, cursor: saveMutation.isPending ? "not-allowed" : "pointer", display: "flex", alignItems: "center", gap: 6 }}>
          {saved ? <><CheckCircle size={14} />Saved</> : saveMutation.isPending ? "Saving..." : "Save"}
        </button>
      </div>
      ...
      <div style={{ padding: "20px 16px", display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <>
            <div>
              ...
              <input type="text" value={form.business_name}
                ...
                placeholder="e.g. Darex Car Rentals" style={inputStyle} />
            </div>
            ...
            <div>
              ...
              <textarea value={form.description}
                ...
                placeholder="Tell customers about your fleet, your services, experience..."
                rows={5} style={{ ...inputStyle, resize: "vertical", lineHeight: 1.6 }} />
            </div>

            {form.address && (
              <div style={{ backgroundColor: ACCENT_BG, borderRadius: 14, padding: "12px 14px" }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: ACCENT, margin: "0 0 2px", textTransform: "uppercase", letterSpacing: "0.06em" }}>Pickup Location</p>
                <p style={{ fontSize: 13, color: "#0A0A0A", margin: 0 }}>{form.address}</p>
                <p style={{ fontSize: 11, color: "#9E9E9E", margin: "4px 0 0" }}>Address is set when your fleet was created by admin</p>
              </div>
            )}

            <div>
              <label style={labelStyle}>Fleet Photos</label>
              <p style={{ fontSize: 11, color: "#9E9E9E", margin: "0 0 8px" }}>First photo becomes your cover on discovery. Add your best fleet shots.</p>
              <ImageUpload
                images={form.images}
        ...
        {activeTab === "Contact" && (
          <>
            <div style={{ backgroundColor: "#E0F7FA", border: "1px solid #80DEEA", borderRadius: 14, padding: "12px 14px" }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#00695C", margin: "0 0 3px" }}>Why this matters</p>
              <p style={{ fontSize: 11, color: "#00796B", margin: 0, lineHeight: 1.5 }}>
                Customers discover your fleet and want to reach you directly. Every field you fill in appears as a contact button on your fleet page.
              </p>
            </div>
            ...
            {[
              ...
              { key: "contact_email", emoji: "✉️", label: "Email Address",    type: "email", placeholder: "fleet@example.com" },
              { key: "website",       emoji: "🌐", label: "Website",          type: "url",   placeholder: "https://yourfleet.com" },
            ...
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, backgroundColor: "#F7F5FA", border: "1.5px solid #E4DCF0", borderRadius: 14, padding: "12px 14px" }}>
                ...
                <input type="text" placeholder="yourfleet" value={form.instagram.replace(/^@/, "")}
            ...
            {(form.whatsapp || form.phone || form.contact_email || form.instagram || form.website) && (
              <div style={{ backgroundColor: "#F7F5FA", borderRadius: 16, padding: "14px" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {form.phone        && <div style={{ display: "flex", alignItems: "center", gap: 6, backgroundColor: ACCENT_BG, border: "1px solid #99F6E4", borderRadius: 10, padding: "7px 12px" }}><span>📞</span><span style={{ fontSize: 12, fontWeight: 700, color: ACCENT }}>Call</span></div>}
        ...
        {activeTab === "Features" && (
          <>
            <div style={{ backgroundColor: "#E0F7FA", border: "1px solid #80DEEA", borderRadius: 14, padding: "12px 14px" }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#00695C", margin: "0 0 3px" }}>Fleet Services</p>
              <p style={{ fontSize: 11, color: "#00796B", margin: 0, lineHeight: 1.5 }}>
                These appear as feature tags on your discovery card and detail page. Select everything your fleet offers.
              </p>
            </div>
            ...
            {form.filters.length > 0 && (
              <p style={{ fontSize: 12, color: "#9E9E9E", textAlign: "center", margin: 0 }}>
                {form.filters.length} service{form.filters.length !== 1 ? "s" : ""} selected
              </p>
            )}
          </>
        )}
        ...
        {activeTab === "Videos" && (
          <>
            <div style={{ backgroundColor: "#E0F7FA", border: "1px solid #80DEEA", borderRadius: 14, padding: "12px 14px" }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#00695C", margin: "0 0 3px" }}>See Our Fleet</p>
              <p style={{ fontSize: 11, color: "#00796B", margin: 0, lineHeight: 1.5 }}>
                Add YouTube, Instagram or TikTok links showcasing your vehicles. These appear in the "See Our Fleet" section on your detail page.
              </p>
            </div>
            <VideoLinksInput
              value={form.videos}
        ...
        <button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}
          style={{ width: "100%", padding: "15px 0", borderRadius: 16, border: "none", backgroundColor: saveMutation.isPending ? "#9E9E9E" : ACCENT, color: "#FFFFFF", fontSize: 15, fontWeight: 700, cursor: saveMutation.isPending ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxShadow: `0 4px 16px rgba(13,148,136,0.3)` }}>
          {saved
