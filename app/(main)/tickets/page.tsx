...
const ACTIVE_STATUSES = ["active", "confirmed", "pending"];
...
function ActiveTicketCard({ ticket, i }: { ticket: any; i: number }) {
...
function PastTicketRow({ ticket, i }: { ticket: any; i: number }) {
  const title = ticket.events?.title || "Event Ticket";
  const image = ticket.events?.images?.[0];
  const status = STATUS_STYLE[ticket.status] || STATUS_STYLE.pending;
        ...
        <div style={{ backgroundColor: "#FFFFFF", borderRadius: 14, display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", border: "1px solid #F0EBF8", opacity: 0.75 }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: "#EDE0F7", overflow: "hidden", flexShrink: 0 }}>
            {image
              ...
              : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #3D0066, #5B0EA6)" }}>
                  <Ticket size={14} style={{ color: "rgba(255,255,255,0.6)" }} />
                </div>}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <p style={{ fontSize: 11, color: "#9E9E9E", margin: 0 }}>
              {ticket.events?.start_date ? format(new Date(ticket.events.start_date), "dd MMM yyyy") : ""}
              {ticket.ticket_type_name ? ` · ${ticket.ticket_type_name}` : ""}
            </p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: "#6B6B6B" }}>{formatCurrency(ticket.amount_paid)}</span>
...
export default function TicketsPage() {
  ...
  const { data: tickets, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return ticketsData.map((t: any) => ({ ...t, events: eventsMap[t.event_id] || null })) as any[];
    },
    enabled: !!user?.id,
    staleTime: 1000 * 30,
  ...
  const active = (tickets || []).filter((t: any) => ACTIVE_STATUSES.includes(t.status));
  const past   = (tickets || []).filter((t: any) => !ACTIVE_STATUSES.includes(t.status));
