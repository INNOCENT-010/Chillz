...
const CATEGORY       = "bar-lounge";
const CATEGORY_LABEL = "Bar & Lounge";
const CATEGORY_EMOJI = "🍸";
const ACCENT         = "#5B0EA6";
const ACCENT_BG      = "#EDE0F7";

const MUSIC_GENRES = ["Afrobeats","Amapiano","Hip-Hop","R&B","EDM","House","Reggae","Live Band"];
const VIBE_TAGS    = ["VIP Table","Rooftop","Live DJ","Hookah / Shisha","Cocktails","18+","Sports Bar","Karaoke"];
const SPEND_RANGES = [
  { label:"Free Entry",  min:0,     max:0 },
  { label:"Under ₦10k", min:1,     max:10000 },
...
function LocationPill({ onCityResolved }:{ onCityResolved:(c:string)=>void }) {
  ...
  useEffect(()=>{
    ...
    navigator.geolocation?.getCurrentPosition(async(pos)=>{
      try{const r=await fetch(`/api/places/geocode?lat=${pos.coords.latitude}&lng=${pos.coords.longitude}`);const d=await r.json();const loc=(d.location||"Lagos").split(",")[0].trim();setDisplay(loc);onCityResolved(loc);}
  ...
  return(
    <div style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer"}}>
      ...
      <span style={{fontSize:12,fontWeight:700,color:"#0A0A0A",maxWidth:90,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>{detecting?"...":display}</span>
...
function Section({title,subtitle,children}:{title:string;subtitle:string;children:React.ReactNode}){
  return(<div style={{marginBottom:28}}><div style={{marginBottom:14}}><h2 style={{fontSize:17,fontWeight:800,color:"#0A0A0A",margin:"0 0 2px",fontFamily:"var(--font-display,Syne,sans-serif)"}}>{title}</h2><p style={{fontSize:11,color:"#9E9E9E",margin:0}}>{subtitle}</p></div>{children}</div>);
}
...
export default function BarLoungePage() {
  ...
  const [quickTonight,setQuickTonight] = useState(false);
  ...
  const [spendIdx,setSpendIdx]       = useState<number|null>(null);
  // applied
  const [appliedGenres,setAppliedGenres]   = useState<string[]>([]);
  const [appliedVibes,setAppliedVibes]     = useState<string[]>([]);
  const [appliedSpend,setAppliedSpend]     = useState<number|null>(null);
  ...
  const {data:savedVenueIds}=useQuery({
    ...
    queryFn:async()=>{
      if(!user?.id)return[];
      const{data}=await(supabase.from("saved_venues")as any).select("venue_id").eq("user_id",user.id);
  ...
  const renderCard=(v:any,i:number)=>(
    <motion.div key={v.id} initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{delay:i*0.05}}>
      <VenueDiscoverCard venue={v} cheapestMenuPrice={v.vendor_id?(menuPrices as any)?.[v.vendor_id]:undefined} categoryEmoji={CATEGORY_EMOJI} accentColor={ACCENT} accentBg={ACCENT_BG}/>
    </motion.div>
  );
      ...
      <div style={{backgroundColor:"#FFFFFF",position:"sticky",top:0,zIndex:40,borderBottom:showFilter?"none":"1px solid #F2EEF9",boxShadow:"0 1px 8px rgba(0,0,0,0.04)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 16px 8px"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <h1 style={{fontSize:20,fontWeight:900,color:"#0A0A0A",margin:0,fontFamily:"var(--font-display,Syne,sans-serif)"}}>{CATEGORY_EMOJI} {CATEGORY_LABEL}</h1>
              <p style={{fontSize:11,color:"#9E9E9E",margin:0}}>{isLoading?"Loading...":`${filtered.length} spot${filtered.length!==1?"s":""} found`}</p>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,padding:"0 16px 12px",overflowX:"auto",scrollbarWidth:"none"}}>
          ...
          <button onClick={()=>{setNearMe(!nearMe);if(!nearMe)navigator.geolocation?.getCurrentPosition((pos)=>{setUserLat(pos.coords.latitude);setUserLng(pos.coords.longitude);},()=>{});}} style={pill(nearMe)}>
            ...
            <motion.div initial={{y:-12,opacity:0}} animate={{y:0,opacity:1}} exit={{y:-12,opacity:0}} transition={{type:"spring",damping:30,stiffness:350}}
              ...
              <div style={{flex:1,overflowY:"auto",padding:"20px 20px 0"}}>
                <div style={{marginBottom:24}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}><p style={{fontSize:13,fontWeight:800,color:"#0A0A0A",margin:0}}>Spend Range</p><button onClick={()=>setSpendIdx(null)} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,fontWeight:700,color:"#EF4444"}}>Clear</button></div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                    {SPEND_RANGES.map((sr,idx)=><button key={sr.label} onClick={()=>setSpendIdx(spendIdx===idx?null:idx)} style={{padding:"8px 16px",borderRadius:999,border:"1.5px solid",borderColor:spendIdx===idx?ACCENT:"#E4DCF0",backgroundColor:spendIdx===idx?ACCENT_BG:"#FFFFFF",color:spendIdx===idx?ACCENT:"#6B6B6B",fontSize:12,fontWeight:spendIdx===idx?700:500,cursor:"pointer"}}>{sr.label}</button>)}
                  </div>
                </div>
                <div style={{marginBottom:24}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}><p style={{fontSize:13,fontWeight:800,color:"#0A0A0A",margin:0}}>Music & Genre</p><button onClick={()=>setGenres([])} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,fontWeight:700,color:"#EF4444"}}>Clear</button></div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                    {MUSIC_GENRES.map(g=>{const on=genres.includes(g);return<button key={g} onClick={()=>setGenres(on?genres.filter(x=>x!==g):[...genres,g])} style={{padding:"8px 14px",borderRadius:999,border:"1.5px solid",borderColor:on?ACCENT:"#E4DCF0",backgroundColor:on?ACCENT_BG:"#FFFFFF",color:on?ACCENT:"#6B6B6B",fontSize:12,fontWeight:on?700:500,cursor:"pointer"}}>{g}</button>;})}
                  </div>
                </div>
                <div style={{marginBottom:24}}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}><p style={{fontSize:13,fontWeight:800,color:"#0A0A0A",margin:0}}>Vibe & Features</p><button onClick={()=>setVibes([])} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,fontWeight:700,color:"#EF4444"}}>Clear</button></div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
                    {VIBE_TAGS.map(vb=>{const on=vibes.includes(vb);return<button key={vb} onClick={()=>setVibes(on?vibes.filter(x=>x!==vb):[...vibes,vb])} style={{padding:"8px 14px",borderRadius:999,border:"1.5px solid",borderColor:on?ACCENT:"#E4DCF0",backgroundColor:on?ACCENT_BG:"#FFFFFF",color:on?ACCENT:"#6B6B6B",fontSize:12,fontWeight:on?700:500,cursor:"pointer"}}>{vb}</button>;})}
                  </div>
                </div>
              </div>
              <div style={{padding:"12px 20px 24px",borderTop:"1px solid #F2EEF9",flexShrink:0}}>
                <button onClick={applyFilter} style={{width:"100%",padding:"14px 0",borderRadius:14,border:"none",backgroundColor:"#0A0A0A",color:"#FFFFFF",fontSize:14,fontWeight:700,cursor:"pointer"}}>Apply Filters</button>
              </div>
            </motion.div></>
          )}
        </AnimatePresence>
      ...
      <div style={{padding:"16px"}}>
        {isLoading?(
          ...
          <div style={{textAlign:"center",paddingTop:80}}>
            ...
            <p style={{fontSize:13,color:"#9E9E9E",margin:"0 0 20px"}}>{showSaved?"Tap the heart on any venue to save it":"Try adjusting your filters"}</p>
            <button onClick={clearAll} style={{padding:"10px 28px",borderRadius:12,border:"none",backgroundColor:ACCENT,color:"#FFFFFF",fontSize:13,fontWeight:700,cursor:"pointer"}}>Clear filters</button>
          </div>
        ):showSaved?(
          <Section title="❤️ Your Saved" subtitle="Bars & lounges you've saved">
          ...
          <>
            {featured.length>0&&<Section title="✨ Featured" subtitle="Hand-picked for you">{featured.map(renderCard)}</Section>}
            {trending.length>0&&<Section title={`🔥 Trending in ${userCity}`} subtitle="Highest rated right now">{trending.map(renderCard)}</Section>}
            {nearby.length>0&&<Section title="📍 Around You" subtitle="Closest to your location">{nearby.map(renderCard)}</Section>}
            {newAdded.length>0&&<Section title="🆕 New on Chillz" subtitle="Recently added">{newAdded.map(renderCard)}</Section>}
          </>
        )}
      </div>
    </MainLayout>
