...
import { Search, X, MapPin, SlidersHorizontal, ArrowLeft,  Heart } from "lucide-react";
...
export default function BarLoungePage() {
  ...
  const fetchPhase = async (
    ...
    return results.filter((v: any) => !currentSeenIds.has(v.id));
  };
  ...
  const initFetch = async (city: string) => {
    ...
    const { neighbourhood, city: cityName } = parseLocation(city);
    // If everywhere, skip to national directly
    ...
    setAllVenues(data);
    const newSeen = new Set(data.map((v:any) => v.id));
    setSeenIds(newSeen);
  ...
  const loadMore = async () => {
    ...
    if (data.length < PAGE_SIZE) {
      ...
      if (data.length < PAGE_SIZE && currentPhase !== "national") {
